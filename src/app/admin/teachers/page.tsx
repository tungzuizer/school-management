    ...
    <div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Sửa giáo viên" : "Thêm giáo viên mới"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!editing && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Mật khẩu (mặc định: 123456)</label>
              <input type="password" value={form.password} onChange={(e) => setForm({...form, password: e.target.value})}
                className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500" placeholder="Để trống sẽ tự đặt là 123456" />
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            <div>
