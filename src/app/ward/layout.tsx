...
const menuGroups: MenuGroup[] = [
  {
    id: "overview",
    code: "01",
    title: "ĐIỀU HÀNH PHÒNG GD&ĐT",
    tag: "Tổng quan",
    accent: "cyan",
    icon: Home,
    items: [
      { label: "Bảng điều khiển", href: "/ward/dashboard", icon: LayoutDashboard, description: "Tổng quan các trường Quận/Huyện" },
    ],
  },
  {
    ...
    icon: Building2,
    items: [
      { label: "Trường MN, TH & THCS", href: "/ward/schools", icon: School, description: "Danh mục trường học trên địa bàn" },
    ],
  },
];

const tagStyles = {
  cyan: "bg-cyan-900/80 text-cyan-200 border-cyan-600/60",
  sky: "bg-sky-900/80 text-sky-200 border-sky-600/60",
};

const codeBadgeStyles = {
  cyan: "bg-cyan-600 text-white border-cyan-400/50 shadow-xs",
...
const cardAccentStyles = {
  cyan: "border-slate-800 hover:border-cyan-700/60 bg-slate-900/80",
...
function WardLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const currentMatchingGroupId = useMemo(() => {
    for (const group of menuGroups) {
      if (
        group.items.some(
          (item) => pathname === item.href || (item.href !== "/ward/dashboard" && pathname.startsWith(item.href))
        )
      ) {
        return group.id;
      }
      ...
      >
        ...
        <div className="p-3.5 border-b border-slate-800/90 bg-[#080d18] shrink-0">
          <div className={`flex items-center ${isCollapsed ? "justify-center" : "gap-3"}`}>
            <div
              className="w-10 h-10 rounded-xl bg-cyan-600 flex items-center justify-center shrink-0 shadow-md text-white font-extrabold text-sm ring-2 ring-cyan-400/20"
              title={isCollapsed ? `${userName} - Phòng GD&ĐT` : undefined}
            >
                ...
                {isCollapsed ? (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 ml-1">
                      ...
                      {isExpanded ? (
                        <ChevronDown className="w-3.5 h-3.5 text-cyan-300 group-hover/hdr:text-white transition-transform" />
                      ) : (
                ...
                {isExpanded && (
                  <div className="space-y-0.5 mt-1">
                    {group.items.map((item) => {
                      const isActive =
                        pathname === item.href ||
                        (item.href !== "/ward/dashboard" && pathname.startsWith(item.href));
                            ...
                            >
                              <Icon
                                className={`w-4 h-4 shrink-0 transition-transform ${
                                  isActive ? "scale-110 text-white" : "text-slate-400 group-hover:scale-110 group-hover:text-cyan-300"
                                }`}
                              />
                            </Link>
                            <NavTooltip
            ...
            <div className="fixed inset-y-0 left-0 w-72 bg-[#0c1322] border-r border-slate-800 z-50 lg:hidden flex flex-col shadow-2xl text-white">
              ...
              <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-[#080d18]">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-cyan-600 text-white font-bold text-xs">
                    {getInitials(userName)}
                  </div>
                  <div className="min-w-0">
                    <p className="text-xs font-bold text-white truncate">{userName}</p>
                    <p className="text-[10px] text-cyan-400 font-semibold truncate">
                      Phòng GD&ĐT
                    </p>
                  </div>
                </div>
                        ...
                        <div className="space-y-0.5">
                          {group.items.map((item) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-2.5 min-w-0">
                                  <Icon className={`w-4 h-4 shrink-0 ${isActive ? "text-white" : "text-slate-400 group-hover:text-cyan-300"}`} />
                                  <span className="truncate">{item.label}</span>
                                </div>
                                {item.badge && (
                                  <span className="px-1.5 py-0.2 rounded text-[9px] uppercase tracking-wider bg-slate-800 text-cyan-300 font-semibold">
                                    {item.badge}
                                  </span>
                                )}
                              </Link>
              ...
              return (
                <Link
                  ...
                  className={`flex flex-col items-center justify-center py-2 px-3 min-w-[64px] min-h-[44px] relative transition-transform duration-200 active:scale-95 ${
                    isActive
                      ? "text-cyan-600 font-bold border-t-2 border-cyan-600"
