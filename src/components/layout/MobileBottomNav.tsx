...
export default function MobileBottomNav({
  ...
  return (
    <nav
      ...
      className="fixed bottom-3 inset-x-3 sm:inset-x-6 sm:bottom-4 z-40 lg:hidden max-w-sm mx-auto select-none"
    >
      {/* iPhone Liquid Glass Pill Bar */}
      <div className="bg-slate-950/70 backdrop-blur-3xl border border-white/20 shadow-[0_16px_40px_-8px_rgba(0,0,0,0.85),inset_0_1px_1px_rgba(255,255,255,0.3)] rounded-full px-3 py-1.5 flex items-center justify-between gap-1 text-slate-300">
        {/* Left 2 Quick Items */}
        {leftItems.map((item) => {
          const isActive =
            pathname === item.href ||
          ...
          return (
            ...
            >
              ...
              />
              <span
                className={`text-[9px] mt-0.5 tracking-tight truncate max-w-[56px] leading-tight transition-colors duration-200 ${
              ...
              {isActive && (
                <span className={`w-1 h-1 rounded-full mt-0.5 absolute -bottom-0.5 ${theme.activeDot}`} />
              )}
            </Link>
          );
        })}
        ...
        <button
          ...
          className="flex-1 flex flex-col items-center justify-center -my-3.5 py-1 px-1 transition-all duration-200 active:scale-90 cursor-pointer group relative"
        >
          <div
            className={`w-11 h-11 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 border ${
              isMenuOpen
                ? theme.menuOpenBtn
                : "bg-slate-900/80 backdrop-blur-2xl text-slate-200 border-white/25 shadow-[0_8px_20px_rgba(0,0,0,0.6),inset_0_1px_1px_rgba(255,255,255,0.35)] hover:bg-slate-800 hover:text-white"
            }`}
          >
            <Menu
          ...
          <span
            className={`text-[9px] mt-0.5 font-extrabold tracking-tight transition-colors duration-200 ${
